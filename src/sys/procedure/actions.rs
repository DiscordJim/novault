...
use crate::{
    console_log, printing::SteppedComputationHandle, sys::{
        ...
        mk::{CachedPassword, MasterVaultKey, UserVaultKey, WrappedKey},
    ...
    fn repair(
        ...
        match source {
            ...
            Self::UnlinkPostSeal => {
                ...
                SEAL_FULL.resume(Self::UnlinkPostSeal).play(root, master)?;
            }
            Self::RelocateEncryptedBinaries => {
                master.starting = false;
                SEAL_FULL
                    .resume(Self::RelocateEncryptedBinaries)
                    .play(root, master)?;
            }
            Self::WriteMandatoryPostSealFiles => {
                master.starting = false;
                SEAL_FULL
                    .resume(Self::WriteMandatoryPostSealFiles)
                    .play(root, master)?;
            }
            Self::RestoreVaultGit => {
                master.starting = false;
                SEAL_FULL.resume(Self::RestoreVaultGit).play(root, master)?;
            }
            ...
            Self::CleanupOldBinaries => {
                master.starting = false;
                UNSEAL_FULL
                    .resume(Self::CleanupOldBinaries)
                    .play(root, master)?;
            }
            Self::RestoreUnsecureFiles => {
                master.starting = false;
                UNSEAL_FULL
                    .resume(Self::RestoreUnsecureFiles)
                    .play(root, master)?;
            }
    ...
    fn _act(&self, root: &RootPath<Normal>, master: &mut Context) -> Result<()> {
        ...
        if master.starting {
            ...
            if !state.is_rest_state() {
                console_log!(
                    Warn,
                    "We are not in a rest state, we were left in an incomplete state ({state:?})."
                );
        ...
        match self {
            ...
            VaultState::ExpandMainVault => {
                expand_decrypted_bin(
                    root.path(),
                    master
                        .decrypted_zip_bytes
                        .take()
                        .ok_or_else(|| anyhow!("Could not get bytes."))?,
                )?;
            }
...
fn expand_decrypted_bin(path: &Path, vault: Vec<u8>) -> Result<()> {
    ...
    for i in 0..real.len() {
        ...
        if entry.is_dir() {
            std::fs::create_dir_all(&out_path).inspect_err(|e| {
                console_log!(
                    Error,
                    "Failed creating a directory (path={out_path:?}): {e:?}"
                );
            })?;
        } else {
            if let Some(parent) = out_path.parent() {
                std::fs::create_dir_all(parent).inspect_err(|e| {
                    console_log!(
                        Error,
                        "Failed creating a parent directory (path={parent:?}): {e:?}"
                    );
                })?;
            }

            let mut outfile = File::create(&out_path).inspect_err(|e| {
                console_log!(Error, "Failed creating a file (path={out_path:?}): {e:?}");
            })?;
            std::io::copy(&mut entry, &mut outfile).inspect_err(|err| {
                console_log!(
                    Error,
                    "Failed copying from buffer to {outfile:?} with error: {err:?}"
                );
            })?;
        }
    }
...
fn stash_external_git_repo(root: &RootPath<Normal>, context: &mut Context) -> Result<()> {
    ...
    if !root.local_git().exists() {
        ...
        if !root.external_git().exists() {
            ...
            return Err(anyhow!(
                "There is no local .git folder, so we checked the wrap folder and there was nothing stashed there either."
            ));
        }
    } else {
    ...
    match master.master.clone() {
        Some(k) => {
            if !root.secure_local_zip().exists() {
                ...
                master.decrypted_local_bytes = Some(
                    decrypt_zip(&root.secure_local_zip(), &k)
                        .map_err(|e| anyhow!("Failed to decrypt zip: {e:?}"))?,
                );
            }
        ...
        None => Err(anyhow!(
            "There was no master key installed, which usually means that the local vault decryption was run prior to the main vault decryption."
        )),
    }
}
...
fn init_filesystem(path: &RootPath<Normal>) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        if unsafe {
            SetFileAttributesW(
                to_wide(met_path.as_os_str()).as_ptr(),
...
/// the new wrapped key.
fn revert_encryption_state(root: &RootPath<Normal>, ctx: &mut Context) -> Result<()> {
...
fn write_encrypted_archives(root: &RootPath<Normal>, ctx: &mut Context) -> Result<()> {
...
fn unlink_other_archives(root: &RootPath<Normal>) -> Result<()> {
    ...
    if !delete.exists() {
        return Err(anyhow!(
            "Failed to find the deletion shards achive, could not proceed with unlinking."
        ));
    }
        ...
        if path.exists() {
            ...
            if path.is_dir() {
                std::fs::remove_dir_all(&path).map_err(|e| {
                    anyhow!("Failed unlinking directory (path={path:?}) with error {e:?}")
                })?;
            } else {
                std::fs::remove_file(&path).map_err(|e| {
                    anyhow!("Failed unlinking file (path={path:?}) with error {e:?}")
                })?;
            }
        }
    }
...
fn restore_vault_git(root: &RootPath<Normal>, ctx: &mut Context) -> Result<()> {
    ...
    std::fs::rename(root.external_git(), root.local_git())?;
